...
export class ExportBackupService {
  ...
  prepararDatosBackup(backups: any[]): any[] {
    return backups.map((backup) => ({
      ID: backup.id,
      'Nombre del Archivo': backup.nombre_archivo,
      'Tamaño (bytes)': backup.tamano_bytes.toLocaleString('es-ES'),
      'Fecha de Backup': new Date(backup.fecha_backup).toLocaleString('es-ES'),
      'Responsable (ID)': backup.usuario_responsable,
      'Responsable (Email)': backup.usuario_responsable_email,
      'Responsable (Nombre)': backup.usuario_responsable_nombre,
      'Tipo de Backup': backup.tipo_backup,
      Estado: backup.estado,
      'Ubicación de Almacenamiento': backup.ubicacion_almacenamiento,
      Notas: backup.notas || 'Sin notas',
    }));
  }
  ...
  private calculateFixedColumnWidths(
    ...
    const fixedWidths: { [key: string]: number } = {
      ...
      'fecha de backup': 30,
      'responsable (id)': 15,
      'responsable (email)': 35,
      'responsable (nombre)': 30,
      'tipo de backup': 25,
      estado: 20,
      'ubicación de almacenamiento': 40,
      notas: 50,
    };
