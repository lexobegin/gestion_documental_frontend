...
export class ExportNotificacionService {
  // Uso:
  // const datos = this.exportNotificacionService.prepararDatosNotificaciones(listaDeNotificaciones);
  // this.exportNotificacionService.exportToPDF(datos, 'reporte_notificaciones', 'Listado de Notificaciones');
  ...
  /** Convertir datos para exportación - NOTIFICACIONES */
  prepararDatosNotificaciones(notificaciones: Notificacion[]): any[] {
    return notificaciones.map((notif) => ({
      ID: notif.id,
      'Usuario ID': notif.usuario,
      'Usuario Nombre': notif.usuario_nombre || 'N/A',
      'Usuario Email': notif.usuario_email || 'N/A',
      'Tipo de Notificación': this.getTipoLabel(notif.tipo),
      Título: notif.titulo,
      Mensaje: notif.mensaje,
      Estado: notif.leida ? 'Leída' : 'No leída',
      'Fecha de Envío': notif.fecha_envio
        ? new Date(notif.fecha_envio).toLocaleString('es-ES')
        : 'N/A',
      'Datos Adicionales': notif.datos_adicionales
        ? JSON.stringify(notif.datos_adicionales)
        : 'No hay datos adicionales',
    }));
  }

  /** Helper para obtener etiquetas de tipo */
  ...
  private calculateFixedColumnWidths(
    ...
    const fixedWidths: { [key: string]: number } = {
      ...
      'usuario nombre': 35,
      'usuario email': 40,
      'tipo de notificación': 25,
      título: 40,
      mensaje: 50,
      estado: 20,
      'fecha de envío': 25,
      'datos adicionales': 40,
    };
