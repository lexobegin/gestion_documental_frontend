...
export class BitacoraService {
  ...
  getBitacora(filters?: BitacoraFilters): Observable<Bitacora[]> {
    let params = new HttpParams();
    if (filters) {
      if (filters.modulo_afectado) params = params.set('modulo_afectado', filters.modulo_afectado);
      if (filters.usuario) params = params.set('usuario', filters.usuario.toString());
